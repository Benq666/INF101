...
import lab4.datastructures.MyGrid;
...
public class LangtonsAnt implements ICellAutomaton {
	...
	public void initializeGeneration() {
		int w = currentGeneration.getWidth();
		int h = currentGeneration.getHeight();
		
		for (int i = 0; i < rule.length; i++) {
			for (int j = 0; j < rule.length; j++) {

				currentGeneration.set(i, j, new CellState(0));
				
			}

		// == er samme objektet. new A() på en side av == vil aldri funke.
		
		ant = new Ant(w/2,h/2,Direction.NORTH);
		...
		*/
		...
		//maur.seenState();
		//currentGeneration.set(w/2, h/2, ) = CellState.DEAD;
		//seenState = cellState.DEAD;
		//farge = Color.;
		//maur.currentGeneration.set(5, 5, NORTH);
